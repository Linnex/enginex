...
if($database->getPartnerStatus($session->userid)==0 && $session->userlevel!=ADMIN_LEVEL && $session->userlevel!=LENDER_LEVEL )
{
	echo "<font style='color:red'>".$lang['brwrlist-i']['inactive_status']."<br /><br />";
	echo $lang['brwrlist-i']['contact_us'];
}
else if($database->getPartnerStatus($session->userid)==1 || $session->userlevel==ADMIN_LEVEL || $session->userlevel==LENDER_LEVEL )
{
	if(!$uid || $uid==0)
	{		
